# 3D object plotter GUI 5/14/2025 CSAIZ
...
def draw():
    global vertices, edges, bodyCenter, flag, spinbox, scale
    flag = True # enables draw figure button
    scale = int(spinbox.get())
    ...
    if filePath != "":
        ...
        messagebox.showwarning("No file", "Please select a file before drawing")

def select_file():
    global filePath
    fileobj = filedialog.askopenfile( title="Select an object file", initialdir= ".", filetypes = (("text file","*.txt"),("all","*.*")))
    filePath = str(fileobj.name)
    print(filePath)

# main ------------------------------------------------------------
def main():
    global root, canvas, flag, spinbox, filePath
    flag = False
    filePath = ""
    # Main window, select file and draw
    menu = Tk()
    menu.title("3D object plotter")
    # Main label
    label = ttk.Label(menu, text = "** 3D PLOTTER **", justify = CENTER, foreground = "blue", background = "yellow", font = ("Courier", 18, "bold"))
    label.grid(row=0, column=0, columnspan=2)
    # Button for object file selection
    fileButton = ttk.Button(menu, text = "Select file", width=10, height=2, command = select_file)
    fileButton.grid(row=1, column=0, columnspan=1)
    # Button for draw action
    drawButton = ttk.Button(menu, text = "draw ", width=10, height=2, command = draw)
    drawButton.grid(row=1, column=1, columnspan=1)
    # Scale factor selector to resize object
    spinLabel = ttk.Label(menu, text = "Scale factor:", justify = CENTER)
    spinLabel.grid(row=2, column=0, columnspan=1)    
    var = IntVar()
    var.set(100)
    spinbox = ttk.Spinbox(menu, from_=1, to=200, increment = 1 ,width=10, relief="sunken", textvariable=var )
    spinbox.grid(row=2, column=1, columnspan=1, padx = 10, pady = 10) 
       
    # Canvas window to draw object
    root = Tk()
    root.title("Canvas plot")    
